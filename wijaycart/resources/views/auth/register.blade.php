    ...
    <div class="card shadow-md">
        <form action="{{ route('register.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" class="input-field @error('name') border-danger @enderror" required>
                @error('name')<p class="mt-1 text-xs text-danger">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="email" class="form-label">Email<span class="form-required">*</span></label>
                <input type="email" id="email" name="email" value="{{ old('email') }}" class="input-field @error('email') border-danger @enderror" required>
            ...
            <div>
                ...
                <x-password-input name="password" id="password" required autocomplete="new-password" class="@error('password') border-danger @enderror" />
                @error('password')<p class="mt-1 text-xs text-danger">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="password_confirmation" class="form-label">Konfirmasi Password<span class="form-required">*</span></label>
        ...
        <p class="mt-6 text-center text-sm text-text/60 dark:text-dark-muted">
            Sudah punya akun? <a href="{{ route('login') }}" class="font-semibold text-accent hover:underline dark:text-primary">Masuk</a>
        </p>
    </div>
</div>
@endsection
