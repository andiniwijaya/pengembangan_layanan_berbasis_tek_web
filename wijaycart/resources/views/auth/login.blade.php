        ...
        <div class="card shadow-md">
            <form action="{{ route('login.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email') }}"
                        class="input-field @error('email') border-danger @enderror" required autofocus>
                    @error('email')
            ...
            <p class="mt-6 text-center text-sm text-text/60 dark:text-dark-muted">
                Belum punya akun? <a href="{{ route('register') }}"
                    class="font-semibold text-accent hover:underline dark:text-primary">Daftar sekarang</a>
            </p>
        </div>
    </div>
@endsection
