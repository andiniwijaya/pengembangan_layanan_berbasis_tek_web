...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-2 text-sm text-text/50 dark:text-dark-muted" aria-label="Breadcrumb">
        ...
        <i data-lucide="chevron-right" class="h-3.5 w-3.5"></i>
        <a href="{{ route('products.index', ['category' => $product->category->slug]) }}" class="hover:text-accent">{{ $product->category->name }}</a>
        <i data-lucide="chevron-right" class="h-3.5 w-3.5"></i>
    ...
    <div class="grid gap-10 lg:grid-cols-2">
        ...
        <div id="product-gallery">
            <div class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm dark:border-dark-border dark:bg-dark-card">
                <img
            ...
            <div class="mt-4 grid grid-cols-4 gap-3 sm:grid-cols-5">
                ...
                >
                    <img src="{{ $image->url }}" alt="Foto {{ $product->name }} {{ $index + 1 }}" class="aspect-square w-full object-cover">
                </button>
                @endforeach
            </div>
            @endif
        ...
        <div class="animate-slide-up">
            ...
            {{-- Rating --}}
            <div class="mb-4">
                @include('partials.product-rating', ['rating' => $product->average_rating, 'count' => $product->review_count, 'size' => 'lg'])
            </div>

            <p class="mb-6 text-3xl font-bold text-accent dark:text-primary">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
            ...
            <div class="mb-6 grid gap-3 rounded-xl bg-secondary/60 p-4 dark:bg-dark-border/40 sm:grid-cols-2">
                ...
                <div class="flex items-center gap-2 text-sm">
                    ...
                    <span class="badge badge-{{ $product->stock_color }}">{{ $product->stock_label }} ({{ $product->stock }})</span>
                </div>
                <div class="flex items-center gap-2 text-sm sm:col-span-2">
                    ...
                    <a href="{{ route('products.index', ['category' => $product->category->slug]) }}" class="font-medium text-accent hover:underline dark:text-primary">{{ $product->category->name }}</a>
                </div>
                @if($product->supplier)
                <div class="flex items-center gap-2 text-sm sm:col-span-2">
            ...
            <div class="mb-8">
                <h2 class="mb-2 text-sm font-semibold uppercase tracking-wider text-accent dark:text-primary">Deskripsi</h2>
                <p class="leading-relaxed text-text/70 dark:text-dark-muted">{{ $product->description }}</p>
            </div>

            @auth
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <form action="{{ route('cart.store') }}" method="POST" class="flex flex-1 items-center gap-3" data-add-to-cart data-ajax="true">
                    @csrf
                    ...
                    <div class="flex items-center rounded-xl border border-border dark:border-dark-border">
                        <button type="button" id="qty-minus" class="px-3 py-3 hover:bg-secondary dark:hover:bg-dark-border" aria-label="Kurangi jumlah">
                            <i data-lucide="minus" class="h-4 w-4"></i>
                        </button>
                        <input type="number" id="quantity" name="quantity" value="1" min="1" max="{{ max(1, $product->stock) }}" class="w-16 border-x border-border bg-transparent py-3 text-center text-sm font-semibold dark:border-dark-border" @disabled(!$product->isInStock())>
                        <button type="button" id="qty-plus" class="px-3 py-3 hover:bg-secondary dark:hover:bg-dark-border" aria-label="Tambah jumlah" @disabled(!$product->isInStock())>
                ...
                <form action="{{ route('wishlist.toggle', $product) }}" method="POST">
                    @csrf
                    <button type="submit" class="btn-secondary px-5 py-3 {{ $inWishlist ? 'border-danger text-danger' : '' }}" aria-label="Wishlist">
    ...
    <section class="mt-16 border-t border-border pt-12 dark:border-dark-border">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            @foreach($relatedProducts as $related)
                @include('partials.product-card', ['product' => $related, 'wishlistIds' => $wishlistIds])
            @endforeach
        </div>
    </section>
    @endif
    ...
    <section class="mt-16 border-t border-border pt-12 dark:border-dark-border">
        ...
        <div class="card mb-6 !border-primary/30">
            ...
            @include('partials.product-rating', ['rating' => $userReview->rating, 'count' => 0, 'size' => 'lg', 'showCount' => false])
        ...
        <div class="card mb-6">
            ...
            <form action="{{ route('reviews.store', $product) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="comment" id="comment" rows="3" class="input-field" maxlength="1000" placeholder="Bagikan pengalaman Anda..."></textarea>
                </div>
                <button type="submit" class="btn-accent">Kirim Ulasan</button>
            </form>
        </div>
        @elseif(auth()->check() && ! $userReview)
        <p class="mb-6 text-sm text-text/60 dark:text-dark-muted">Anda dapat memberikan ulasan setelah pesanan produk ini selesai (Delivered).</p>
        @endif
        ...
        <div class="space-y-4">
            ...
            <article class="card !p-4">
                <div class="mb-2 flex items-center justify-between gap-2">
                    ...
                    @include('partials.product-rating', ['rating' => $review->rating, 'count' => 0, 'size' => 'sm', 'showCount' => false])
                </div>
                @if($review->comment)
                <p class="text-sm text-text/70 dark:text-dark-muted">{{ $review->comment }}</p>
                @endif
                <p class="mt-2 text-xs text-text/40 dark:text-dark-muted">{{ $review->created_at->format('d M Y') }}</p>
            </article>
            @empty
