        ...
        <div class="flex flex-wrap gap-3">
            ...
            <div class="relative">
                ...
                <form action="{{ route('admin.product-images.destroy', $image) }}" method="POST" class="absolute -right-2 -top-2">
                    @csrf @method('DELETE')
                    <button type="submit" class="rounded-full bg-danger p-1 text-white"><i data-lucide="x" class="h-3 w-3"></i></button>
                </form>
            </div>
            @endforeach
        </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="supplier_id" class="input-field">
                        ...
                        <option value="{{ $supplier->id }}" @selected(old('supplier_id', $product->supplier_id) == $supplier->id)>{{ $supplier->code }} — {{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium">Barcode</label>
                    <input type="text" name="barcode" value="{{ old('barcode', $product->barcode) }}" class="input-field font-mono">
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium">Harga (Rp)</label>
                    <input type="number" name="price" value="{{ old('price', $product->price) }}" class="input-field" min="0" required>
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $product->is_featured)) class="rounded text-primary focus:ring-primary">
                        <span class="text-sm">Featured Product</span>
                    </label>
                </div>
            </div>
            <div class="flex gap-3">
                <button type="submit" class="btn-accent">Update Produk</button>
