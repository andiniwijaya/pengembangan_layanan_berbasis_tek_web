...
@section('content')
    ...
    @if ($orders->isEmpty())
        <x-empty-state icon="shopping-bag" title="Belum Ada Transaksi"
            description="Pesanan dari customer akan muncul di sini setelah checkout." />
    @else
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <td><span class="badge badge-{{ $order->status_color }}">{{ $order->status_label }}</span>
                                </td>
                                <td>
                                    @if ($order->payment)
                                        <span
                                            class="badge {{ $order->payment->status === 'paid' ? 'badge-success' : 'badge-warning' }}">{{ $order->payment->status_label }}</span>
                                    @else
