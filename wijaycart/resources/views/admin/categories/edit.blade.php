        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-5">
            ...
            <div class="rounded-xl border border-border/60 bg-secondary/30 p-4 dark:border-dark-border dark:bg-dark-card/40">
                ...
                <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow-sm dark:bg-dark-card">
                    <i data-lucide="{{ $category->icon }}" class="h-8 w-8 text-accent dark:text-primary" aria-hidden="true"></i>
                </div>
                <p class="mt-2 text-xs text-text/50 dark:text-dark-muted">Ikon Lucide otomatis berdasarkan slug: <code class="text-accent">{{ $category->slug }}</code></p>
            </div>
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $category->is_active)) class="rounded text-primary focus:ring-primary">
                <span class="text-sm">Aktif</span>
            </label>
            <div class="flex gap-3">
                <button type="submit" class="btn-accent">Update</button>
