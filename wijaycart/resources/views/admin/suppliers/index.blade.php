...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <form action="{{ route('admin.suppliers.index') }}" method="GET" class="flex flex-wrap gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, kode, email..." class="input-field w-full sm:w-64">
    ...
    <a href="{{ route('admin.suppliers.create') }}" class="btn-accent text-sm"><i data-lucide="plus" class="h-4 w-4"></i> Tambah Supplier</a>
</div>

<p class="mb-4 text-sm text-text/60 dark:text-dark-muted">{{ $suppliers->total() }} supplier</p>

@if($suppliers->isEmpty())
<x-empty-state icon="truck" title="Belum Ada Supplier" description="Tambahkan supplier untuk melengkapi data master produk." :action-url="route('admin.suppliers.create')" action-label="Tambah Supplier" />
@else
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex gap-1">
                            ...
                            <form action="{{ route('admin.suppliers.destroy', $supplier) }}" method="POST" class="inline" data-confirm="Supplier akan dihapus permanen. Lanjutkan?" data-confirm-title="Hapus Supplier">
                                @csrf @method('DELETE')
                                <x-icon-action icon="trash-2" tooltip="Hapus Supplier" type="submit" variant="danger" />
                            </form>
                        </div>
                    </td>
                </tr>
