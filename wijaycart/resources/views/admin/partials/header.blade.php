<header
    ...
    <div class="flex h-16 items-center justify-between gap-4 px-4 md:px-6">
        ...
        <div class="flex shrink-0 items-center gap-1 sm:gap-2">
            ...
            <div id="tooltip-admin-theme" role="tooltip"
                class="tooltip invisible absolute z-50 inline-block rounded-lg bg-accent px-3 py-1.5 text-xs font-medium text-white opacity-0 shadow-lg transition-opacity duration-300 dark:bg-dark-border">
                Mode Gelap
                <div class="tooltip-arrow" data-popper-arrow></div>
            </div>

            <div class="relative ml-1" data-dropdown-toggle="admin-profile-dropdown"
                ...
                <button type="button"
                    ...
                    <span
                        class="hidden max-w-[120px] truncate text-sm font-medium lg:inline">{{ auth()->user()->name }}</span>
                    <i data-lucide="chevron-down" class="hidden h-4 w-4 text-text/50 lg:inline" aria-hidden="true"></i>
                </button>
                <div id="admin-profile-dropdown"
                    class="z-50 hidden w-52 divide-y divide-border rounded-xl border border-border bg-card shadow-lg dark:divide-dark-border dark:border-dark-border dark:bg-dark-card">
                    <div class="px-4 py-3">
                    ...
                    <ul class="py-1 text-sm">
                        <li>
                            <a href="{{ route('profile.edit') }}"
                                class="flex items-center gap-2 px-4 py-2.5 hover:bg-secondary dark:hover:bg-dark-border">
                    ...
                    <div class="py-1">
                        <form action="{{ route('logout') }}" method="POST" data-confirm="Anda yakin ingin keluar?"
