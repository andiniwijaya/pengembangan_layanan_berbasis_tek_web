...
<article class="product-card group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-dark-border dark:bg-dark-card">
    {{-- Foto Produk --}}
    <div class="relative aspect-square overflow-hidden bg-secondary dark:bg-dark-border">
        ...
        <span class="absolute left-3 top-3 badge badge-warning">Featured</span>
        @endif
    ...
    <div class="flex flex-1 flex-col p-4">
        {{-- Kategori --}}
        <p class="mb-1 text-xs font-semibold uppercase tracking-wider text-accent/80 dark:text-primary/80">{{ $product->category->name }}</p>

        {{-- Nama Produk --}}
        <a href="{{ route('products.show', $product->slug) }}" class="mb-2 line-clamp-2 font-semibold text-text transition-colors hover:text-accent dark:text-dark-text dark:hover:text-primary">
            {{ $product->name }}
        </a>

        {{-- Rating --}}
        @include('partials.product-rating', ['rating' => $product->average_rating, 'count' => $product->review_count])

        {{-- Harga --}}
        <p class="mb-1 text-lg font-bold text-accent dark:text-primary">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
        ...
        <div class="mt-auto flex flex-col gap-2">
            ...
            <div class="flex gap-2">
                <form action="{{ route('cart.store') }}" method="POST" class="flex-1" data-add-to-cart data-ajax="true">
                    @csrf
