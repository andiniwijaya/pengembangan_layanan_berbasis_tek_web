...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <a href="{{ route('orders.index') }}" class="card-hover group flex items-center gap-4">
            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/30 transition-transform group-hover:scale-105">
        ...
        <a href="{{ route('wishlist.index') }}" class="card-hover group flex items-center gap-4">
            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-danger/15 transition-transform group-hover:scale-105">
        ...
        <a href="{{ route('cart.index') }}" class="card-hover group flex items-center gap-4">
            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-accent/15 transition-transform group-hover:scale-105">
        ...
        <div class="space-y-8 lg:col-span-2">
            ...
            <section aria-labelledby="recent-orders-heading">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <a href="{{ route('orders.index') }}" class="text-sm font-medium text-accent hover:underline dark:text-primary">Lihat Semua</a>
                </div>
                ...
                @else
                    <div class="card divide-y divide-border dark:divide-dark-border !p-0 overflow-hidden">
                        ...
                        <a href="{{ route('orders.show', $order) }}" class="flex items-center justify-between px-5 py-4 transition-colors hover:bg-secondary/50 dark:hover:bg-dark-border/30">
                            <div>
                                <p class="font-semibold">{{ $order->order_number }}</p>
                                <p class="text-xs text-text/50 dark:text-dark-muted">{{ $order->created_at->format('d M Y') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-accent dark:text-primary">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
        ...
        <div class="space-y-8">
            ...
            <section class="card" aria-labelledby="address-heading">
                ...
                @if($user->address)
                    ...
                    @endif
                    <a href="{{ route('profile.edit') }}" class="mt-4 inline-flex text-sm font-medium text-accent hover:underline dark:text-primary">Ubah Alamat</a>
                @else
                    <x-empty-state
                        icon="map-pin"
                        title="Alamat Belum Diisi"
                        description="Lengkapi alamat agar checkout lebih cepat."
                        :action-url="route('profile.edit')"
            ...
            <section aria-labelledby="popular-categories-heading">
                ...
                @if($popularCategories->isEmpty())
                    <x-empty-state
                        ...
                        description="Kategori produk akan muncul di sini."
                        :action-url="route('products.index')"
                ...
                @else
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('products.index', ['category' => $category->slug]) }}" class="card-hover flex items-center gap-3 !p-3">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-border/40 dark:bg-dark-card dark:ring-dark-border">
                                <i data-lucide="{{ $category->icon }}" class="h-6 w-6 text-accent dark:text-primary" aria-hidden="true"></i>
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="truncate font-semibold">{{ $category->name }}</p>
                                <p class="text-xs text-text/50 dark:text-dark-muted">{{ $category->active_products_count }} produk</p>
                            </div>
                            <i data-lucide="chevron-right" class="h-4 w-4 shrink-0 text-text/40" aria-hidden="true"></i>
                        </a>
                        @endforeach
                    </div>
                @endif
